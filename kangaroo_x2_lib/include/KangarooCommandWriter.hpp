...
class KangarooCommandWriter {
 ...
 public:
  size_t writeToBuffer(uint8_t* buffer, uint8_t address,
                       KangarooCommand command) const;
  static size_t writeToBuffer(uint8_t* buffer, uint8_t address,
                              KangarooCommand command, const uint8_t* data,
                              size_t lengthOfData);
  void writeToStream(Stream& port, uint8_t address,
                     KangarooCommand command) const;
  static void writeToStream(Stream& port, uint8_t address,
                            KangarooCommand command, const uint8_t* data,
                            size_t lengthOfData);
